...
export default function CappuccinoPage({ data }) {
  const drinks = data.allContentfulDrinkRecipe.nodes

  return (
    <Layout>
      <h1>Cappuccino Recipes</h1>
      <p>Browse hot cappuccino drinks.</p>
...
export const query = graphql`
  {
    allContentfulDrinkRecipe(
      filter: { temperature: { eq: "hot" }, category: { eq: "cappuccino" } }
      sort: { title: ASC }
    ) {
      nodes {
        title
...
export const Head = () => <Seo title="Cappuccino Recipes" />