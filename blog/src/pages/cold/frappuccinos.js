...
export default function FrappuccinosPage({ data }) {
  const drinks = data.allContentfulDrinkRecipe.nodes

  return (
    <Layout>
      <h1>Frappuccino Recipes</h1>
      <p>Browse cold frappuccino drinks.</p>
...
export const query = graphql`
  {
    allContentfulDrinkRecipe(
      filter: { temperature: { eq: "cold" }, category: { eq: "frappuccinos" } }
      sort: { title: ASC }
    ) {
      nodes {
        title
...
export const Head = () => <Seo title="Frappuccino Recipes" />