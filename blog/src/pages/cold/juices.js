...
export default function JuicesPage({ data }) {
  const drinks = data.allContentfulDrinkRecipe.nodes

  return (
    <Layout>
      <h1>Juice Recipes</h1>
      <p>Browse cold juice drinks.</p>
...
export const query = graphql`
  {
    allContentfulDrinkRecipe(
      filter: { temperature: { eq: "cold" }, category: { eq: "juices" } }
      sort: { title: ASC }
    ) {
      nodes {
        title
...
export const Head = () => <Seo title="Juice Recipes" />