...
export default function SmoothiesPage({ data }) {
  const drinks = data.allContentfulDrinkRecipe.nodes

  return (
    <Layout>
      <h1>Smoothie Recipes</h1>
      <p>Browse cold smoothie drinks.</p>
...
export const query = graphql`
  {
    allContentfulDrinkRecipe(
      filter: { temperature: { eq: "cold" }, category: { eq: "smoothies" } }
      sort: { title: ASC }
    ) {
      nodes {
        title
...
export const Head = () => <Seo title="Smoothie Recipes" />